...
export default function Items({ wishlistId, ownerId, role }: { wishlistId: number, ownerId?: number, role?: string }) {
  ...
  const [form, setForm] = useState<Partial<Item>>({ name: '', url: '', description: undefined, image_url: undefined, price: undefined });
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    try {
      // try to fetch metadata from the URL if present and no explicit description/image/price
      let metadata: any = {};
      if (form.url && (!form.description && !form.image_url && !form.price)) {
        try {
          const m = await api.get('/metadata/fetch', { params: { url: form.url } });
          metadata = m.data || {};
        } catch (e) {
          // ignore metadata fetch errors, proceed with basic data
        }
      }
      const payload = { ...form, wishlist_id: wishlistId, description: form.description ?? metadata.description, image_url: form.image_url ?? metadata.image, price: form.price ?? metadata.price };
  ...
  const startEdit = (item: Item) => {
    setEditing(item);
    setForm({ name: item.name, url: item.url, description: item.description, image_url: item.image_url, price: item.price });
  };
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.put(`/items/${editing.id}`, { name: form.name, url: form.url, description: form.description, image_url: form.image_url, price: form.price });
      setItems(items.map(i => i.id === res.data.id ? res.data : i));
      setEditing(null);
      setForm({ name: '', url: '', description: undefined, image_url: undefined, price: undefined });
    } catch (e) {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setOpen(false)}>
          <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-2xl max-w-md w-full mx-4 shadow-2xl border border-gray-200 dark:border-gray-700" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{t('Image URL')}</label>
                <input
                  type="url"
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{t('Price')}</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">{t('Seul le nom et le lien sont nécessaires. Le reste sera récupéré automatiquement, ou vous pouvez le saisir manuellement.')}</p>
            </div>
